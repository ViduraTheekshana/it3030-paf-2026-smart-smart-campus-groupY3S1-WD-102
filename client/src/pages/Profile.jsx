...
function StrengthBar({ password }) {
	...
	return (
		<div style={{ marginTop: 6 }}>
			...
			<p style={{ fontSize: 11, color: colors[s], fontWeight: 500 }}>
				{labels[s]}
			</p>
		</div>
	);
...
export default function Profile() {
	...
	// 1) currentUser = backend user directly
	// 2) currentUser = { accessToken, tokenType, user }
	const authUser = currentUser?.user || currentUser || {};
	...
	async function handleSave() {
		...
		setSaving(true);
		try {
			// Adjust this endpoint if your backend uses a different route/body for profile update
			...
			const updatedUser = data?.user ||
				data || { ...authUser, fullName: form.fullName.trim() };
	...
	async function handlePhotoChange(e) {
		...
		if (file.size > 5 * 1024 * 1024)
			return showToast("File size must be under 5 MB", "error");
		if (!userId)
			return showToast("Session error — please log in again", "error");
		...
		setUploading(true);
		try {
			// Adjust this endpoint if your backend uses a different route/body for photo upload
			...
			const newUrl =
				data?.profilePictureUrl || data?.photoUrl || data?.url || null;
	...
	async function handleChangePw() {
		...
		setPwSaving(true);
		try {
			// Adjust this endpoint if your backend uses a different route/body for password change
			await axios.post(
								...
								<div style={S.fieldRow}>
									...
									{editMode ? (
										<>
											...
											/>
											{errors.fullName && (
												<p style={S.errText}>{errors.fullName}</p>
											)}
										</>
									) : (
								...
								<div style={S.fieldRow}>
									...
									>
										...
										<span style={S.fieldValue}>
											{getProviderLabel(authUser?.provider)}
										</span>
									</div>
								</div>
							</div>
										...
										>
											{[
												...
												<div key={key}>
													...
													{pwErrors[key] && (
														<p style={S.errText}>{pwErrors[key]}</p>
													)}
													{key === "next" && (
							...
							<div style={S.card}>
								...
								>
									{[
										...
										{
											label: "Status",
											value: authUser?.enabled ? "Active" : "Disabled",
											green: !!authUser?.enabled,
										},
